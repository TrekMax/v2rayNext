...
# bootstrap wget wrapper (no proxy support yet — proxy set after lib.sh)
_wget() {
    wget "$@"
}
...
# root
[[ $EUID != 0 ]] && err "当前非 ${red}ROOT用户.${none}"

# yum or apt-get, ubuntu/debian/centos
cmd=$(type -P apt-get || type -P yum)
[[ ! $cmd ]] && err "此脚本仅支持 ${red}(Ubuntu or Debian or CentOS)${none}."

# systemd
[[ ! $(type -P systemctl) ]] && {
    err "此系统缺少 ${red}(systemctl)${none}, 请尝试执行:${red} ${cmd} update -y;${cmd} install systemd -y ${none}来修复此错误."
}
...
# --- install.sh-specific functions ---

# print a message (install-time version; full msg() comes from lib.sh)
msg() {
    case $1 in
    warn)
        local color='\e[33m'
...
get_ip() {
    ...
    [[ -z $ip ]] && ip=$(_wget -6 -qO- https://one.one.one.one/cdn-cgi/trace | grep -oP 'ip=\K[0-9a-fA-F.:]+')
}
...
pass_args() {
    while [[ $# -gt 0 ]]; do
        case $1 in
        online)
            err "如果想要安装旧版本, 请转到: https://github.com/TrekMax/v2rayNext/tree/old"
            ;;
        -f | --core-file)
            [[ -z $2 ]] && {
            ...
            ;;
        -l | --local-install)
            [[ ! -f ${PWD}/src/init.sh || ! -f ${PWD}/$is_core.sh ]] && {
...
main() {
    ...
    [[ -f $is_sh_bin && -d $is_core_dir/bin && -d $is_sh_dir && -d $is_conf_dir ]] && {
        err "检测到脚本已安装, 如需重装请使用${red} ${is_core} reinstall ${none}命令."
    }

    # check parameters
    [[ $# -gt 0 ]] && pass_args "$@"

    # enable proxy for _wget after args parsed
    [[ $proxy ]] && _wget() { export https_proxy=$proxy; wget "$@"; }
    ...
    # start installing...
    msg warn "开始安装..."
    [[ $is_core_ver ]] && msg warn "${is_core_name} 版本: ${is_core_ver}"
    [[ $proxy ]] && msg warn "使用代理: ${proxy}"
    # create tmpdir
    mkdir -p "$tmpdir"
    # if is_core_file, copy file
    [[ $is_core_file ]] && {
        cp -f $is_core_file $is_core_ok
        msg warn "${is_core_name} 文件使用 > $is_core_file"
    }
    # local dir install sh script
    [[ $local_install ]] && {
        >$is_sh_ok
        msg warn "本地获取安装脚本 > $PWD"
    }

    timedatectl set-ntp true &>/dev/null
    [[ $? != 0 ]] && {
        msg warn "提醒!!! 无法设置自动同步时间, 可能会影响使用 VMess 协议."
    }
    ...
    # source lib.sh now that the script dir is in place
    . $is_sh_dir/src/lib.sh
    ...
    if [[ $jq_not_found ]]; then
        chmod +x "$is_jq_ok"
        if ! "$is_jq_ok" --version &>/dev/null; then
            msg err "jq 文件完整性校验失败"
            exit_and_del_tmpdir
        fi
